...
from tensorflow.keras.applications.vgg19 import preprocess_input # Penting untuk VGG19
import numpy as np
...
# --- KONFIGURASI MODEL ---
MODEL_PATH = os.path.join('model', 'final_best_model_vgg19_finetuned.h5') 
IMAGE_SIZE = (224, 224) 
CLASS_NAMES = ['Chickenpox', 'Measles', 'Monkeypox', 'Normal'] 

# Muat model saat aplikasi startup
try:
    model = load_model(MODEL_PATH)
    print(f"Model berhasil dimuat dari {MODEL_PATH}")
except Exception as e:
    print(f"Error memuat model: {e}")
    model = None # Handle jika model gagal dimuat
